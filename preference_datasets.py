...
def strip_html_tags(html_string):
    """Strip HTML tags from a string, except for <code> tags (which contain real
    code in the StackExchange answers)."""
...
def get_se(
    ...
    """Load the StackExchange dataset from Huggingface, and return a dict of prompts
    and responses. See get_hh for the format.

    We strip the HTML tags from the responses (except for <code> tags), and we add
    necessary newlines.
    """
...
def get_shp(
    ...
    """Load the Stanford Human Preferences dataset from Huggingface and convert it to
    the necessary format. See hh for the format.
...
def get_hh(
    ...
    """Load the Anthropic Helpful-Harmless dataset from Huggingface and convert it to
    ...
    Prompts should be structured as follows:
      \n\nHuman: <prompt>\n\nAssistant:
    Multiple turns are allowed, but the prompt should always start
    with \n\nHuman: and end with \n\nAssistant:.
...
def get_dataset(name: str, split: str, silent: bool = False, cache_dir: str = None):
    ...
    if name == "":
        ...
        get_custom_shp_dataset_from_fp(fp, silent=False, verbose=False)
...
def get_batch_iterator(
    ...
    """Get an iterator over batches of data. Stops after n_epochs or n_examples,
